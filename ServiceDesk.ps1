...
function SchoolMatcher{
    ...
    if($matchme -eq $false)
    {
        ...
        if($matchme -eq $true)
        {
            ...
            if($Matches.School -eq "KAIE"){$SchoolLoc = "KTown ES"       ; $RoomNum = $Matches.Room ; $OU = "OU=Production,OU=Computers,OU=Kaiserslautern Germany,OU=East,OU=Europe,OU=Regional Resources,DC=DS,DC=DODEA,DC=EDU" ; $OldOU = "OU=Windows10,OU=Computers,OU=Kaiserslautern ES,OU=Schools and Offices,DC=eu,DC=DS,DC=DODEA,DC=EDU"    ; $MDrive = "" ; $IDrive = "" ; $HomeDrive = "" }#KTown ES
            if($Matches.School -eq "KAIH"){$SchoolLoc = "KTown HS"       ; $RoomNum = $Matches.Room ; $OU = "OU=Production,OU=Computers,OU=Kaiserslautern Germany,OU=East,OU=Europe,OU=Regional Resources,DC=DS,DC=DODEA,DC=EDU" ; $OldOU = "OU=Windows10,OU=Computers,OU=Kaiserslautern HS,OU=Schools and Offices,DC=eu,DC=DS,DC=DODEA,DC=EDU"    ; $MDrive = "" ; $IDrive = "" ; $HomeDrive = "" }#KTown HS
            if($Matches.School -eq "KAIM"){$SchoolLoc = "KTown MS"       ; $RoomNum = $Matches.Room ; $OU = "OU=Production,OU=Computers,OU=Kaiserslautern Germany,OU=East,OU=Europe,OU=Regional Resources,DC=DS,DC=DODEA,DC=EDU" ; $OldOU = "OU=Windows10,OU=Computers,OU=Kaiserslautern MS,OU=Schools and Offices,DC=eu,DC=DS,DC=DODEA,DC=EDU"    ; $MDrive = "" ; $IDrive = "" ; $HomeDrive = "" }#KTown MS
            if($Matches.School -eq "LAND"){$SchoolLoc = "Landstuhl ES/MS"; $RoomNum = $Matches.Room ; $OU = "OU=Production,OU=Computers,OU=Kaiserslautern Germany,OU=East,OU=Europe,OU=Regional Resources,DC=DS,DC=DODEA,DC=EDU" ; $OldOU = "OU=Windows10,OU=Computers,OU=Landstuhl EMS,OU=Schools and Offices,DC=eu,DC=DS,DC=DODEA,DC=EDU"        ; $MDrive = "" ; $IDrive = "" ; $HomeDrive = "" }#Landstuhl ES/MS
            if($Matches.School -eq "VOGE"){$SchoolLoc = "Vogelweh ES"    ; $RoomNum = $Matches.Room ; $OU = "OU=Production,OU=Computers,OU=Kaiserslautern Germany,OU=East,OU=Europe,OU=Regional Resources,DC=DS,DC=DODEA,DC=EDU" ; $OldOU = "OU=Windows10,OU=Computers,OU=Vogelweh ES,OU=Schools and Offices,DC=eu,DC=DS,DC=DODEA,DC=EDU"          ; $MDrive = "" ; $IDrive = "" ; $HomeDrive = "" }#Vogelweh ES
            if($Matches.School -eq "RAME"){$SchoolLoc = "Ramstein ES"    ; $RoomNum = $Matches.Room ; $OU = "OU=Production,OU=Computers,OU=Ramstein AB Germany,OU=East,OU=Europe,OU=Regional Resources,DC=DS,DC=DODEA,DC=EDU"    ; $OldOU = "OU=Windows10,OU=Computers,OU=Ramstein ES,OU=Schools and Offices,DC=eu,DC=DS,DC=DODEA,DC=EDU"          ; $MDrive = "" ; $IDrive = "" ; $HomeDrive = "" }#Ramstein ES
            if($Matches.School -eq "RAMH"){$SchoolLoc = "Ramstein HS"    ; $RoomNum = $Matches.Room ; $OU = "OU=Production,OU=Computers,OU=Ramstein AB Germany,OU=East,OU=Europe,OU=Regional Resources,DC=DS,DC=DODEA,DC=EDU"    ; $OldOU = "OU=Windows10,OU=Computers,OU=Ramstein HS,OU=Schools and Offices,DC=eu,DC=DS,DC=DODEA,DC=EDU"          ; $MDrive = "" ; $IDrive = "" ; $HomeDrive = "" }#Ramstein HS
            if($Matches.School -eq "RAMI"){$SchoolLoc = "Ramstein IS"    ; $RoomNum = $Matches.Room ; $OU = "OU=Production,OU=Computers,OU=Ramstein AB Germany,OU=East,OU=Europe,OU=Regional Resources,DC=DS,DC=DODEA,DC=EDU"    ; $OldOU = "OU=Windows10,OU=Computers,OU=Ramstein IS,OU=Schools and Offices,DC=eu,DC=DS,DC=DODEA,DC=EDU"          ; $MDrive = "" ; $IDrive = "" ; $HomeDrive = "" }#Ramstein IS
            if($Matches.School -eq "RAMM"){$SchoolLoc = "Ramstein MS"    ; $RoomNum = $Matches.Room ; $OU = "OU=Production,OU=Computers,OU=Ramstein AB Germany,OU=East,OU=Europe,OU=Regional Resources,DC=DS,DC=DODEA,DC=EDU"    ; $OldOU = "OU=Windows10,OU=Computers,OU=Ramstein MS,OU=Schools and Offices,DC=eu,DC=DS,DC=DODEA,DC=EDU"          ; $MDrive = "" ; $IDrive = "" ; $HomeDrive = "" }#Ramstein MS
            #South
            if($Matches.School -eq "BRUD"){$SchoolLoc = "Brussels EHS"   ; $RoomNum = $Matches.Room ; $OU = "OU=Production,OU=Computers,OU=Brussels Belgium,OU=West,OU=Europe,OU=Regional Resources,DC=DS,DC=DODEA,DC=EDU"       ; $OldOU = "OU=Windows10,OU=Computers,OU=Brussels EHS,OU=Schools and Offices,DC=eu,DC=DS,DC=DODEA,DC=EDU"         ; $MDrive = "\\ds.dodea.edu\Europe\BRUS-BRUS"  ; $IDrive = "\\ds.dodea.edu\Europe\BRUS-InfoDrive"  ; $HomeDrive = "\\ds.dodea.edu\Europe\BRUS-HOME"}#Brussels MS
            $op.OldName  = $CompName
            $op.NewName  = ""
            $op.Location = $SchoolLoc
            $op.Room     = $RoomNum
        ...
        {
            if ($NoNewName.IsPresent){"No match"}
            else{
                $Form.TopMost = $false
                ...
                if($Matches.School -eq "RAMM"){$SchoolLoc = "Ramstein MS"    ; $RoomNum = $Matches.Room ; $OU = "OU=Production,OU=Computers,OU=Ramstein AB Germany,OU=East,OU=Europe,OU=Regional Resources,DC=DS,DC=DODEA,DC=EDU"    ; $OldOU = "OU=Windows10,OU=Computers,OU=Ramstein MS,OU=Schools and Offices,DC=eu,DC=DS,DC=DODEA,DC=EDU"          ; $MDrive = "" ; $IDrive = "" ; $HomeDrive = "" }#Ramstein MS
                #South
                if($Matches.School -eq "BRUD"){$SchoolLoc = "Brussels EHS"   ; $RoomNum = $Matches.Room ; $OU = "OU=Production,OU=Computers,OU=Brussels Belgium,OU=West,OU=Europe,OU=Regional Resources,DC=DS,DC=DODEA,DC=EDU"       ; $OldOU = "OU=Windows10,OU=Computers,OU=Brussels EHS,OU=Schools and Offices,DC=eu,DC=DS,DC=DODEA,DC=EDU"         ; $MDrive = "\\ds.dodea.edu\Europe\BRUS-BRUS"  ; $IDrive = "\\ds.dodea.edu\Europe\BRUS-InfoDrive" ; $HomeDrive = "\\ds.dodea.edu\Europe\BRUS-HOME" }#Brussels EHS
                $op.OldName  = $CompName
...
function ComputerCleanup{
    ...
    Try{
        ...
        $complist = Get-ADComputer -Filter ('(OperatingSystem -eq "Windows 10 Enterprise") -and lastLogonTimestamp -gt 0') -SearchBase 'CN=Computers,DC=DS,DC=DODEA,DC=EDU' -SearchScope OneLevel -Properties Name,DistinguishedName,OperatingSystem
        if($complist -ne $null){
            Logger -FilePath $LogFileLoc -Text "Found matching computer objects, moving them to the proper OU now..." -OPWindow
            Foreach($comp in $complist){  
                $SchoolMatcher = SchoolMatcher -CompName $comp.Name -NoNewName
                if ($SchoolMatcher -ne "No Match" -and $SchoolMatcher.OU -ne $null){
                    Logger -FilePath $LogFileLoc -Text "Moving $($comp.Name) to $($SchoolMatcher.OU)" -OPWindow
                    try{
                        Move-ADObject -Identity $comp.DistinguishedName -TargetPath $SchoolMatcher.OU
                        Logger -FilePath $LogFileLoc -Text "Success" -OPWindow
                    }
                    catch{Logger -FilePath $LogFileLoc -Text "Failed in moving $($comp.Name) to $($SchoolMatcher.OU), with $($Error[0])" -OPWindow}
                }
            }
        }
    ...
    Logger -FilePath $LogFileLoc -Text "Copmleted cleaning up the Computers OU" -OPWindow
}
...
} #Add in ability to generate for specific user rather than whoever is logged on.
...
"@
    ...
    try{Remove-Item -Path "C:\Users\$CurrentUserName\ElevateScript.txt" -Force | Out-Null}
    catch{}
    ...
    $Form.add_FormClosing({
        ...
        try{
            ...
            Remove-Item -Path "C:\Users\$CurrentUserName\ElevateScript.txt" -Force | Out-Null
            Remove-Item "C:\Users\$CurrentUserName\RenameRemoteComputers.bat" -Force
